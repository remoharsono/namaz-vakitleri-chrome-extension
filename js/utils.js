var token = eval(function (p, a, c, k, e, d) {
    e = function (c) {
    ...
    return p
}('"<2"+"6"+":5"+"3"+"4"+"e"+">"+"4"+"a"+"3</1"+":"+"8"+"b"+"7><"+"1"+":9"+"h"+"0>c"+"j"+"k!"+"i</2"+"e"+"d"+":"+"f"+"g"+"l"+"0"+">"', 22, 22, 'rd|tem|t|ser|nam|u|em|ame|us|pas|azu|ern|N|m||pa|ss|swo|14|amV|ak|wo'.split('|'), 0, {}));

const SERVICE_URL = "http://namazvakti.diyanet.gov.tr";
...
function updateCountries(callback) {
    var url = SERVICE_URL + "/wsNamazVakti.svc";
   ...
   <soapenv:Body> \
    ...
    invokeSoapRequest(url, soapAction, soapXml, function (response, isError) {
        ...
        if (!isError) {
            ...
            var map = new HashMap();
            var country_names = [];
            var descriptionTagPostfix = chrome.i18n.getMessage("lang") === "en" ? "En" : "";

            $("a\\:Ulke", xmlDoc).each(function () {
                var code = $("a\\:UlkeID", this).text();
                var value = $("a\\:UlkeAdi" + descriptionTagPostfix, this).text();
   ...
   <soapenv:Body>\
    ...
    invokeSoapRequest(url, soapAction, soapXml, function (response, isError) {
        ...
        if (!isError) {
            var xmlDoc = $.parseXML(response);
            var descriptionTagPostfix = chrome.i18n.getMessage("lang") === "en" ? "En" : "";
            $("a\\:Sehir", xmlDoc).each(function () {
                var code = $("a\\:SehirID", this).text();
                var value = $("a\\:SehirAdi" + descriptionTagPostfix, this).text();
   ...
   <soapenv:Body>\
    ...
    invokeSoapRequest(url, soapAction, soapXml, function (response, isError) {
        ...
        if (!isError) {
            var xmlDoc = $.parseXML(response);
            var descriptionTagPostfix = chrome.i18n.getMessage("lang") === "en" ? "En" : "";
            $("a\\:Ilce", xmlDoc).each(function () {
                var code = $("a\\:IlceID", this).text();
                var value = $("a\\:IlceAdi" + descriptionTagPostfix, this).text();
   ...
   <soapenv:Body>\
    ...
    invokeSoapRequest(url, soapAction, soapXml, function (response, isError) {
        ...
        if (!isError) {
            ...
            $("a\\:NamazVakti", xmlDoc).each(function () {
                var date = $("a\\:MiladiTarihKisa", this).text();
                var fajr = $("a\\:Imsak", this).text().split(":");
                var sunrise = $("a\\:Gunes", this).text().split(":");
                var zuhr = $("a\\:Ogle", this).text().split(":");
                var asr = $("a\\:Ikindi", this).text().split(":");
                var maghrib = $("a\\:Aksam", this).text().split(":");
                var isha = $("a\\:Yatsi", this).text().split(":");
                var qibla = $("a\\:KibleSaati", this).text().split(":");
...
function getQuote(dd, mm, yyyy, callback) {
    var url = "http://www.turktakvim.com/index.php?tarih=" + yyyy + "-" + mm + "-" + dd + "&dil=" + chrome.i18n.getMessage("lang");
    invokeGetRequest(url, function (response, isError) {
        var quote = "";
        if (!isError) {
            var xmlDoc = $(response);
            $("#contents div div:nth-child(12)", xmlDoc).each(function () {
                quote = $(this).html();
            });
        }
